...
html = re.sub(r'<button class="send-btn"[^>]*>.*?</button>', new_button_html, html, flags=re.DOTALL)
...
# First remove the old .send-btn rules we added for the circular gray button
css = re.sub(r'\.send-btn\s*\{.*?(?=\.send-btn >|\Z)', '', css, flags=re.DOTALL)
css = re.sub(r'\.send-btn > svg\s*\{.*?\}', '', css, flags=re.DOTALL)
css = re.sub(r'\.send-btn:hover\s*\{.*?\}', '', css, flags=re.DOTALL)
css = re.sub(r'\.send-btn:hover > svg\s*\{.*?\}', '', css, flags=re.DOTALL)

# Add the new animated CSS
new_css = """
.button {
  --primary: #ffffff;
  --neutral-1: rgba(255,255,255,0.05);
  --neutral-2: rgba(255,255,255,0.15);
  --radius: 20px;
...
/* States */
.state { padding-left: 20px; z-index: 2; display: flex; position: relative; }
